 ...
 */
const loadRoutes = (): Record<string, RouteDataType> => {
  try {
 ...
 */
const handleRequest = (req: Request, res: Response, route: RouteDataType) => {
  // Check parameters
  for (const [paramKey, paramValue] of Object.entries(route.params ?? {})) {
  ...
  // Check request body
  if (Object.keys(route.body ?? {}).length > 0 && !isEqual(req.body, route.body ?? {})) {
 ...
 */
const configureMockEndpoints = (): void => {
 ...
 */
const isEqual = (obj1: Record<string, string>, obj2: Record<string, string>): boolean => {
